...
| `GET /cases/{caseId}/audit-logs` | `{ data: [AuditLog] }` chronological case history & audit events |
| `GET /audit-logs` | `{ data: [AuditLog] }` organization/department-scoped audit ledger |
...
The `AuditLog` object tracks every status transition, forensic note, AI evaluation, and evidence interaction:

| Field | Type | Description |
| --- | --- | --- |
| `id` | `string` (UUID) | Unique identifier of the audit log record |
| `caseRecordId` | `string` (UUID) | ID of the case record this log belongs to |
| `actorType` | `string` | Originator of the action: `DEPARTMENT_HEAD`, `AI`, or `SYSTEM` |
| `action` | `string` | Audit action type: `STATUS_CHANGED`, `AI_PROCESSED`, `EVIDENCE_REVIEWED`, `EVIDENCE_ADDED`, `MESSAGE_SENT`, `ESCALATED` |
| `previousValue` | `string \| null` | Previous state value (e.g. previous status before transition) |
| `newValue` | `string \| null` | Target state value (e.g. new status or uploaded evidence ID) |
| `note` | `string \| null` | Mandatory forensic or investigative note provided by the officer |
| `loggedAt` | `string` (ISO 8601) | Exact timestamp of the event in UTC |

Endpoint permissions:
- `GET /cases/{caseId}/audit-logs`: Available to Department Heads (assigned or eligible within department) and Managers.
- `GET /audit-logs`: Available to all authenticated staff; Department Heads receive logs for their departmental cases, Managers receive organization-wide logs.
